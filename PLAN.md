...
- [x] **Decouple zone re-emit from OCR fire cadence (2026-05-19, `5c0c1d2`).** Main caches the last `HoverZonePayload` and re-emits it on a renderer-initiated `hover:resync` ping. Renderer fires the ping whenever `hoverMode` is true (initial mount, toggle-back-on, dev-reload remount). Cache cleared on `captureSetSource`/`captureStop` so a stale payload from a previous source can't be replayed.
...
### Ship 3 — Sentence mining — **CODE COMPLETE, DOGFOOD PENDING** (2026-05-19, `5a543a0`)

Most of the work was already done by Ship 2.8 — the `latestFrame` latch in main carries the cropped PNG, the hover-zone payload carries the line text and the hovered token, and the VLM-driven dictionary path means the mined sentence/word uses the corrected transcription rather than Vision's first-pass. Remaining work was wiring those into an AnkiConnect call behind a hotkey.

- [x] **AnkiConnect v6 client in main process** (`src/main/anki/client.ts`). HTTP to `127.0.0.1:8765`, typed errors (`AnkiUnreachableError`, `AnkiDuplicateError`, `AnkiError`). Startup probe via `ankiVersion()` so the log indicates connectivity up-front.
- [x] **One-hotkey card creation on `⌘⇧M`**. Renderer holds live hover + translation state, packages a `SubmitToAnkiRequest` when the hotkey fires; main reconciles against `latestFrame` (frameId match), crops via existing `cropAroundLine`, calls `storeMediaFile` + `addNote`. No-hover case mines the focused line if a fresh translation exists.
- [x] **Card template config** at `~/Library/Application Support/yomiko/anki-config.json`. Defaults to jp-mining-note shape (Word/WordReading/WordMeaning/Sentence/SentenceTranslation/Picture). Field map is user-overridable.
- [ ] **First-run AnkiConnect permission flow** — detect AnkiConnect running, prompt for permission grant in Anki, surface error states clearly. *Open: not actually exercised because the dogfood never ran (vacation, no Anki + AnkiConnect to test against).*
- [ ] **End-to-end dogfood.** Open Anki + AnkiConnect, hover a token in TestVN (or a real VN), `⌘⇧M`, verify card lands with all six fields populated and the line crop visible. The plan at `~/.claude/plans/silly-waddling-book.md` enumerates the verification steps.
...
### Ship 3.5 — Aesthetic + UX polish (in progress)

Driven by `/impeccable critique` against `src/renderer/src` on 2026-05-20 (snapshot lives at `.impeccable/critique/2026-05-20T14-51-12Z__src-renderer-src.md`, gitignored). The original critique scored the overlay 22/40 with one P1 ("glass-card monoculture") that's now shipped, plus the items below.

**Shipped this batch:**

- [x] **TokiMemo-leaning aesthetic pass** (`5d989a9`). Pixel-box `.vnr-panel` chrome with `clip-path` corner cuts; tinted-dark surfaces (warm `oklch(0.18 0.012 350)`); three text tiers; pastel-shifted accent palette (rose / mint / amber / lavender); Maru Gothic in the JP stack; pulsing-dot loading affordance replacing the gradient shimmer; status strip + translation overlay treated as ambient surfaces (no card chrome). `DESIGN.md` captures the language. Replaces the prior `bg-black/85 + backdrop-blur + rounded-lg + shadow-2xl` monoculture across five surfaces.
- [x] **Translation-overlay dwell-gate**. New `dwelledLineKey` state in `HoverProtoLayer`; overlay only mounts after the 250ms dwell completes on a stable `(frameId, lineIdx)` pair. Cursor passing through hover zones never flashes content. Re-hovers of an already-dwelled line restore the cached translation instantly.
- [x] **Suppress hover when captured app isn't focused** (`b10534e`). `vendor/macos-window-info/main.swift` reports `frontmost: bool` via owner-PID match against the first kCGWindowLayer-0 window. Main polls every 500ms when `activeSourceWindowId` is parseable; emits `Channels.sourceFocusChanged`. Renderer gates the dwell timer + overlay render on `sourceFocused`. Stops the click-through overlay from triggering VLM fetches while the user is in Slack / a terminal / etc.
- [x] **VLM status pill** in the status strip with green / amber states and tooltips pointing at `settings.json` keys when offline (`9822c01`).
- [x] **`⇧ dict` discoverability hint** appended to the hover-mode pill so users can find the Shift-gated JMdict popup (`55e4257`).

**Outstanding (in priority order):**

- [ ] **[P1] Hotkey discoverability beyond `⇧ dict`.** Five hotkeys exist (`⌘⇧M` mining, `⌘⇧T` force-translate, `⌘⇧H` hover-toggle, `⌘⇧D` debug rects, `⌘⇧J` vertical); only `⇧ dict` and an in-overlay `⌘⇧T to dismiss` are surfaced. Mining is Ship 3's headline feature and is invisible to new users. Candidate fixes: a `?` affordance in the status strip that pops a compact reference card; or a one-time first-run overlay listing the hotkeys; or just extend the status-strip pill to enumerate them all.
- [ ] **[P2] Engineer-register copy.** `VLM ready` / `VLM offline` / `connected` / `no source` leak the engineering model. A JP-learner audience has no idea what "VLM" is. Quick rename pass — keep technical terms in tooltips, surface user-readable labels.
- [ ] **[P2] State-color audit.** The aesthetic pass introduced semantic accent tokens (`--accent-rose / mint / amber / lavender`), but a grep for residual `text-cyan-300/70`, `text-red-200`, `bg-emerald-400/20` instances would tighten consistency. Suspected stragglers: `Popup.tsx` PoS pill, `SourcePicker.tsx` error banner.
- [ ] **[P2] Opacity ladder cleanup.** Three text tiers are defined in `main.css` (`--text-primary / secondary / tertiary`). Audit for residual `text-white/40|45|50|60|80|85|90` instances in the renderer and collapse to the three tiers.
- [ ] **Minor:** `z-index` ladder is scattered (`z-50` / `z-[100]` in different places); worth tokenizing. `RegionSelector.tsx` still uses the pre-aesthetic glass chrome (`rounded-xl border-white/10 bg-black/90 backdrop-blur`); only hit if you go through the atypical-layout escape hatch in the picker, but should be aligned eventually. `SourcePicker.tsx` first-run tooltip ("Looking for windows... macOS will prompt for Screen Recording access...") is a three-step parenthetical that should be a guided state. The TestVN window's `Test VN` title is dev-only but visible in the source picker.
- [ ] **Re-run `/impeccable critique`** once the above land. The May-20 snapshot scored 22/40 against a state that no longer exists; the trend will be meaningful only against fresh runs.
...
- **Vertical (tategaki) text via Apple Vision is parked** (2026-05-11). Vision is unreliable on vertical Japanese — frequently returns zero observations even on clear, legible vertical text. The natural fix is to pre-rotate the captured crop 90° CCW before sending to Vision (renderer-side canvas-to-canvas drawImage on the cropCanvas to a rotated target canvas). That worked in the early Ship-2.5-era experiment on the bench, but the current Electron 39 + ScreenCaptureKit stack produces persistent Skia "non-existent mailbox" errors on the canvas-to-canvas drawImage path — the rotated output is silently blank, OCR ends up running against either nothing or the original chrome. Tried `willReadFrequently: true` on all three canvases to force CPU-backed storage; Skia errors persisted, no fix. The supported path for vertical text is `⌘⇧T` (force-translate the full frame to the VLM) — the VLM doesn't care about orientation and handles vertical Japanese natively. The hover-on-vertical-text feature is parked. Reopen if a future Electron/Chromium release stabilizes the canvas-to-canvas GPU mailbox lifecycle, or if a manual-pixel-rotation (`getImageData` → JS loop → `putImageData`, no drawImage) becomes worth the per-frame CPU cost.

## Picking up where we left off (handoff for the next session)

If you're a fresh Claude Code session opening this repo (last refreshed 2026-05-20 after `b10534e`):

1. **Ships 1, 2, 2.5, 2.7, 2.8, and 3 are merged to `main`.** Ship 2.6 was investigated and killed as not viable (vocab-gap finding; see closeout). Ship 3 is code-complete but the AnkiConnect end-to-end dogfood is still pending (vacation, no Anki available). Aesthetic + focus-gating polish (Ship 3.5 partial) shipped at `5d989a9` and `b10534e`. All commits at `https://github.com/jrgmadrid/yomiko` (private). Local working dir clean.
2. **Run it:** `npm install && npm run build:dict && npm run dev`. The Mac sidecars (Vision OCR + window-info) autobuild on postinstall via `npm run build:sidecar:mac`. Swift toolchain required. Win OCR sidecar source is at `vendor/windows-media-ocr/` but uncompiled. Build canary is `npm run build` (runs `typecheck:node` + `typecheck:web` + vite build); use that, not `tsc --build` (which clobbers `src/preload/index.d.ts` — see the [[no-tsc-build]] gotcha lower in this section).
3. **The test rig:** in DevTools console of the overlay window, `window.vnr.openTestVN()` opens a fake VN with eight Japanese lines (→ / Space to advance). Pick it in the source picker — picker auto-closes on first frame, no region drag. Hover over textbox text → translation overlay (after 250ms dwell); hold ⇧ for the JMdict popup. `⌘⇧D` shows debug rects, `⌘⇧T` force-translates the full frame, `⌘⇧M` mines to Anki, `⌘⇧H` toggles hover mode. The Test VN path uses `webContents.capturePage()` (see "Dogfood notes #1") because SCK throttles non-interacted owned BrowserWindows.
4. **For real windows:** pick anything (Safari with JP Wikipedia, Notes.app with JP text, etc.). Frames flow through `getDisplayMedia` → OCRSource → `frame-ocr` → `emitRealWindowHoverZones`. Window position comes from the macos-window-info sidecar (`CGWindowListCopyWindowInfo`). The sidecar also reports whether the captured window's owner PID matches the topmost layer-0 window's PID; main polls every 500ms and suppresses hover-driven translation when the captured app isn't focused.
5. **The actually-open tasks**, in rough priority order:
   - **Ship 3 dogfood**: install AnkiConnect, exercise `⌘⇧M`, verify card creation per the verification plan at `~/.claude/plans/silly-waddling-book.md`. First-run permission flow likely needs adjustment after.
   - **Ship 3.5 P1 — hotkey discoverability**: mining hotkey is invisible to new users. See Ship 3.5 section.
   - **Ship 3.5 P2 cluster — copy + state-color + opacity ladder cleanup.** Cheap individually, additive in effect.
   - **Crossover testing** (waiting on PC access): Textractor-WS end-to-end + Textractor escape-hatch validation + Crossover-window capture. See the "Crossover unblocks" notes lower in plan.
   - **Windows parity** (deferred until Win box / time): sidecar bbox extension + `DwmGetWindowAttribute` via koffi. Ship 2.5 leftover.
6. **Don't touch the architecture.** Specifically: (a) don't reintroduce the bottom-bar overlay or standalone popup BrowserWindow refactor (Dogfood note #2); (b) don't switch the Vision sidecar to `.fast` for per-character bboxes (Ship 2.5 finding #1: zero observations on Japanese); (c) don't move the `onHoverZones` subscription back into `HoverProtoLayer` (Ship 2.5 finding #2: payload must survive hover-mode remounts); (d) don't restart the manga-OCR investigation without reading the Ship 2.6 closeout above first — the vocab-gap finding kills any constrained-vocab JP OCR (manga-OCR, EasyOCR, PaddleOCR all confirmed missing 唵); (e) **<a id="no-tsc-build"></a>don't run `npx tsc --build` or `tsc --build --force`** — `composite: true` without an `outDir` causes tsc to emit `.d.ts` / `.js` files into `src/`, *overwriting* `src/preload/index.d.ts` with a single `export {};` line. Use `npm run typecheck` (per-project, `--noEmit`) or `npm run build`.
7. **Don't touch the design language without reading `DESIGN.md`.** TokiMemo-leaning pixel-box chrome with `clip-path` corner cuts on panel surfaces; warm-tinted-dark surfaces; three text tiers; pastel accent palette; pulsing-dot loading. `.vnr-panel` is the panel utility; ambient surfaces (translation overlay, status strip) skip it. New surfaces should adopt the existing tokens, not invent new ones — that's how we got the original AI-slop monoculture.
8. **Order of operations going forward:** finish Ship 3.5 (hotkey discoverability + copy + audits) → Crossover testing (when on PC) → Ship 4 (settings/polish + Textractor toggle) → Windows parity → Ship 5 (distribution). Rare-kanji substitution stays a documented limit; revisit only as a cloud-VLM fallback (see Open questions).
